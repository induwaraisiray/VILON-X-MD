...
cmd({
    pattern: "mode",
    alias: ["setmode"],
    desc: "Set bot mode to private, public, group or pm.",
    category: "settings",
    filename: __filename,
}, async (conn, mek, m, { from, args, isCreator, reply, isGroup }) => {

    if (!isCreator)
        return reply("*📛 Only the owner can use this command!*");

    // If no args provided → show current mode
    if (!args[0]) {
    ...
    if (["private", "public", "groups", "pm"].includes(modeArg)) {
        config.MODE = modeArg;

        return reply(
            modeArg === "private" ? "🔒 Bot is now in *PRIVATE MODE* (only owner)." :
            modeArg === "public"  ? "🌐 Bot is now in *PUBLIC MODE* (everyone)." :
            modeArg === "groups"   ? "👥 Bot is now in *GROUPS MODE* (groups only)." :
            "📩 Bot is now in *PM MODE* (private chat only)."
        );
    } else {
        return reply("❌ Invalid mode.\nUse: `.mode public`, `.mode private`, `.mode groups`, `.mode pm`");
    }
});
...
async (conn, mek, m, { from, args, isCreator, reply }) => {
    ...
    if (status === "on") {
        ...
        await reply(`*🛠️ ᴇxᴀᴍᴘʟᴇ: .ᴀɴᴛɪᴅᴇʟᴇᴛᴇ ᴏɴ*`);
    }
});
